...
export function AuditReport({data,clientView=false}:{data:ReportData;clientView?:boolean}){
  ...
  return <article className="audit-report mx-auto max-w-6xl space-y-8 text-foreground">
    ...
    <Section title="Constraint diagnosis" eyebrow="04 · Diagnosis"><div className="space-y-4">{data.diagnoses.map((d,i)=><div key={d.id} className="rounded-2xl border border-border/60 bg-ink-deep p-5"><div className="flex items-start gap-4"><span className="flex h-9 w-9 shrink-0 items-center justify-center rounded-xl bg-mint/10 font-black text-mint">{i+1}</span><div><h3 className="font-black text-ice">{d.title}</h3><p className="mt-2 text-sm leading-6 text-muted-foreground">{d.statement}</p><div className="mt-3"><EvidenceBadge classification={d.classification} confidence={d.confidence}/></div></div></div></div>)}</div></Section>

    <Section title="Opportunity register" eyebrow="05 · Priorities"><div className="space-y-4">{data.opportunities.map((o,i)=><div key={o.id} className="grid gap-4 rounded-2xl border border-border/60 bg-ink-deep p-5 md:grid-cols-[1fr_140px]"><div><p className="text-xs font-bold uppercase tracking-[0.12em] text-mint">Priority {i+1}</p><h3 className="mt-2 text-xl font-black text-ice">{o.title}</h3><p className="mt-2 text-sm leading-6 text-muted-foreground">{o.description}</p>{o.estimated_impact_pence!=null?<p className="mt-3 text-sm font-bold text-ice">Modelled impact: {money(o.estimated_impact_pence)}</p>:null}</div><div className="rounded-xl border border-border bg-ink p-4 text-center"><p className="text-[10px] font-bold uppercase tracking-[0.14em] text-muted-foreground">Opportunity score</p><p className="mt-2 font-display text-4xl font-black text-mint">{number(o.overall_score)}</p><p className="mt-1 text-[10px] text-muted-foreground">{o.score_version}</p></div></div>)}</div></Section>

    <Section title="Top recommendations & 90-day plan" eyebrow="06 · Action"><div className="grid gap-4 lg:grid-cols-3">{data.recommendations.map(r=><div key={r.id} className="rounded-2xl border border-border/60 bg-ink-deep p-5"><p className="text-[10px] font-black uppercase tracking-[0.16em] text-mint">{r.phase}</p><h3 className="mt-2 text-lg font-black text-ice">{r.title}</h3><p className="mt-3 text-sm leading-6 text-muted-foreground">{r.action}</p><p className="mt-3 text-xs leading-5 text-muted-foreground"><strong className="text-ice">Mechanism:</strong> {r.mechanism}</p><p className="mt-3 text-xs text-muted-foreground">Owner: {r.owner||"Studio + INKSIGHTS"}{r.target_metric?` · KPI: ${r.target_metric}`:""}</p></div>)}</div></Section>
