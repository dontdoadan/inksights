...
function RevenueAuditPage() {
  ...
  async function submit(event: FormEvent<HTMLFormElement>) {
    ...
    try {
      ...
      setError(
        err instanceof Error ? err.message : "The audit could not be generated. Please try again.",
      );
    } finally {
      setLoading(false);
    }
        ...
        <div className="mx-auto max-w-5xl px-6 py-16 md:py-24">
          <div className="max-w-3xl">
            ...
            <p className="mt-6 max-w-2xl text-lg leading-relaxed text-muted-foreground md:text-xl">
              Give us a few operating numbers and INKSIGHTS will calculate a first-pass opportunity
              estimate across unused capacity, unconverted enquiries, cancellations and repeat
              clients.
            </p>
      ...
      <section className="mx-auto grid max-w-7xl gap-8 px-6 py-12 lg:grid-cols-[1fr_320px] lg:py-16">
        <form onSubmit={submit} className="rounded-3xl border border-border bg-ink p-6 md:p-9">
          <div className="space-y-10">
            ...
            <div>
              ...
              <h2 className="mt-2 font-display text-2xl font-black text-ice">
                Use your average month.
              </h2>
              <p className="mt-2 text-sm text-muted-foreground">
                Approximate figures are fine. The more accurate the inputs, the more useful the
                estimate.
              </p>
              <div className="mt-5 grid gap-4 sm:grid-cols-2">
                <Field
                  label="Monthly studio revenue (£)"
                ...
                />
                <Field
                  label="Available artist hours / month"
                ...
                />
                <Field
                  label="Booked artist hours / month"
            ...
            <div>
              ...
              <h2 className="mt-2 font-display text-2xl font-black text-ice">
                Where might money be escaping?
              </h2>
              <div className="mt-5 grid gap-4 sm:grid-cols-2">
                <Field
                  label="Repeat-client rate (%)"
                  name="repeat_client_rate"
                  type="number"
                  min="0"
                  max="100"
                  placeholder="35"
                ...
                <label className="text-sm font-semibold text-ice">
                  ...
                  <select
                    ...
                    className="mt-2 w-full rounded-xl border border-border bg-ink-deep px-4 py-3 font-normal text-ice"
                  >
            ...
            <div>
              ...
              <div className="mt-5 rounded-2xl border border-mint/20 bg-mint/5 p-5">
                <div className="flex gap-3">
                  ...
                  <p className="text-sm leading-relaxed text-muted-foreground">
                    Your result is generated from the figures you provide. We will clearly
                    distinguish an estimate from verified studio data.
                  </p>
                </div>
              </div>
              ...
              <label className="mt-5 flex gap-3 text-sm text-muted-foreground">
                ...
                <span>
                  I agree to INKSIGHTS using these details to generate and follow up this Studio
                  Growth Check. <b className="text-mint">Required.</b>
                </span>
              </label>
              ...
              <button
                disabled={loading}
                className="mt-6 inline-flex min-h-12 items-center gap-2 rounded-full bg-mint px-6 py-3 font-bold text-ink-deep transition hover:bg-mint-soft disabled:cursor-wait disabled:opacity-60"
                type="submit"
              >
                {loading ? "Calculating…" : "Generate my Studio Growth Check"}{" "}
        ...
        <aside className="h-max rounded-3xl border border-border bg-ink p-6 lg:sticky lg:top-24">
          ...
          <div className="mt-6 rounded-2xl border border-border bg-ink-deep p-4 text-xs leading-relaxed text-muted-foreground">
            <b className="text-ice">Important:</b> This free check estimates opportunities from your
            answers; it does not verify lost revenue. The paid Full INKSIGHTS Audit is a separately
            scoped review of available studio evidence. The diagnostic engine and connected data
            workflows are in active build.
          </div>
        </aside>
      </section>
        ...
        <section id="audit-result" className="border-t border-border bg-ink">
          <div className="mx-auto max-w-7xl px-6 py-14 md:py-20">
            <div className="max-w-4xl">
              <p className="text-xs font-bold uppercase tracking-[0.16em] text-mint">
                Your Studio Growth Check
              </p>
              <h2 className="mt-3 font-display text-4xl font-black tracking-tight text-ice md:text-6xl">
                Estimated opportunity:{" "}
                <span className="text-mint">
                  {currency(result.estimate.annual_low)}–{currency(result.estimate.annual_high)}
                </span>{" "}
                / year
              </h2>
              <p className="mt-4 text-lg leading-relaxed text-muted-foreground">
                The largest estimated opportunity in your inputs is{" "}
                <b className="text-ice">{result.estimate.primary_opportunity}</b>.
              </p>
            </div>
            <div className="mt-10 grid gap-4 md:grid-cols-2 xl:grid-cols-4">
              {result.findings.map((finding) => (
                <div
                  key={finding.label}
                  className="rounded-2xl border border-border bg-ink-deep p-5"
                >
                  <p className="text-sm font-bold text-ice">{finding.label}</p>
                  <p className="mt-3 font-display text-2xl font-black text-mint">
                    {currency(finding.annual_low)}–{currency(finding.annual_high)}
                  </p>
            ...
            <div className="mt-8 grid gap-6 lg:grid-cols-[1fr_.8fr]">
              <div className="rounded-2xl border border-border bg-ink-deep p-6">
                ...
                <ol className="mt-5 space-y-4">
                  {result.recommendations.map((item, i) => (
                    ...
                    >
                      <span className="font-mono text-mint">0{i + 1}</span>
                      <span>{item}</span>
                    </li>
                  ))}
                </ol>
              ...
              <div className="rounded-2xl border border-mint/20 bg-mint/5 p-6">
                ...
                <h3 className="mt-2 font-display text-2xl font-black text-ice">
                  Want the estimate replaced with real data?
                </h3>
                <p className="mt-3 text-sm leading-relaxed text-muted-foreground">
                  Discuss a paid Full INKSIGHTS Audit of the data you can provide. Scope, price and
                  data requirements are agreed before work begins. Broader integrations are planned
                  after the full diagnostic loop is validated on one real business.
                </p>
                <a
                  href="mailto:hello@getinksights.co.uk?subject=Revenue%20Audit%20V1%20follow-up"
                  className="mt-5 inline-flex items-center gap-2 rounded-full bg-mint px-5 py-3 text-sm font-bold text-ink-deep"
                >
                  Discuss the Full INKSIGHTS Audit <ArrowRight className="h-4 w-4" />
                </a>
              </div>
            </div>
