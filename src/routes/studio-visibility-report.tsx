...
type Keyword = { keyword: string; search_volume: number; current_rank: number | null; lsos_score?: number | null; target_url?: string | null; keyword_difficulty?: number | null };
type ReportPayload = {
...
function ReportView({ data }: { data: ReportPayload }) {
  ...
  return <PublicShell>
    ...
    <main className="bg-ink print:bg-white">
      ...
      <section className="mx-auto max-w-7xl px-6 py-14"><div className="grid gap-8 lg:grid-cols-[1fr_380px]"><div><p className="text-xs font-bold uppercase tracking-[0.18em] text-mint">Search evidence</p><h2 className="mt-2 font-display text-4xl font-black text-ice">Observed search positions</h2><div className="mt-7 overflow-hidden rounded-2xl border border-border"><div className="grid grid-cols-[1fr_110px_90px_90px] bg-ink-deep px-5 py-4 text-xs font-bold uppercase tracking-wider text-muted-foreground"><span>Search</span><span>Volume</span><span>Rank</span><span>Index</span></div>{[...kws].sort((a,b) => Number(b.search_volume)-Number(a.search_volume)).map(k => <div key={k.keyword} className="grid grid-cols-[1fr_110px_90px_90px] items-center border-t border-border px-5 py-4 text-sm"><span className="font-semibold text-ice">{k.keyword}</span><span className="text-muted-foreground">{Number(k.search_volume).toLocaleString("en-GB")}</span><span className="font-bold text-ice">{k.current_rank == null ? "Not in top 100" : `#${k.current_rank}`}</span><span className="font-bold text-mint">{typeof k.lsos_score === "number" ? k.lsos_score.toFixed(1) : "—"}</span></div>)}</div></div>
