import { IntelligenceMethodology } from "@/components/business-progress";
...
const constraints = [
  {
    ...
    summary:
      "Your portfolio may be strong, but the studio is difficult to discover or weakly positioned when people search locally.",
    symptoms: [
      "Weak Google visibility",
      "Inconsistent local signals",
      "Poor search-to-profile conversion",
      "Traffic that does not match your preferred work",
    ],
  ...
  },
  {
    ...
    summary:
      "Demand exists, but vague intake, slow responses or unclear next steps create leakage between interest and deposit.",
    symptoms: [
      "High enquiry volume",
      "Low enquiry-to-booking conversion",
      "Repeated questions in DMs",
  ...
  },
  {
    ...
    summary:
      "The studio has available capacity, but the diary is not being filled with the right projects at the right value.",
    symptoms: [
      "Gaps between appointments",
      "Unclear utilisation",
      "Last-minute filling of the diary",
      "No consistent capacity baseline",
    ],
    route: "/offers/revenue-audit",
  ...
  },
  {
    ...
    summary:
      "The relationship effectively ends at checkout instead of becoming repeat projects, referrals or future bookings.",
    symptoms: [
      "Low repeat-booking rate",
      "Dormant client lists",
      "No reactivation process",
      "Inconsistent review or referral requests",
    ],
  ...
  },
  {
    ...
    summary:
      "Reporting shows totals but not the commercial conditions producing them or the constraint most worth fixing next.",
    symptoms: [
  ...
  head: () => ({
    meta: [
      { title: "Tattoo Studio Growth Solutions UK | INKSIGHTS" },
      {
        name: "description",
        content:
          "Find the right tattoo studio growth solution for visibility, enquiry conversion, capacity, retention or revenue control.",
      },
      { property: "og:title", content: "Tattoo Studio Growth Solutions UK | INKSIGHTS" },
      {
        property: "og:description",
        content:
          "Start with the constraint. Choose the smallest practical intervention for the studio's actual problem.",
      },
    ...
    <PublicShell>
      <JsonLd
        data={{
          ...
          url: CANONICAL_URL,
          description:
            "Growth and commercial solutions for UK tattoo studios organised around the constraint being experienced.",
        }}
      />
      <PageHero
        ...
        description={
          <>
            Start with a free Studio Growth Check, then discuss a paid Full INKSIGHTS Audit if
            deeper evidence is needed. The diagnostic engine and growth playbooks are in active
            build for UK tattoo studios.
          </>
        }
      >
        <a
          href="/studio-growth-check"
          className="inline-flex min-h-12 items-center gap-2 rounded-full bg-mint px-6 py-3 font-bold text-ink-deep"
        >
          Run the free Studio Growth Check <ArrowRight className="h-4 w-4" />
        </a>
        <a
          ...
          <div className="mt-10 grid gap-5 md:grid-cols-2 xl:grid-cols-3">
            {constraints.map((item) => {
              ...
              return (
                <article
                  key={item.title}
                  className="group flex h-full flex-col rounded-2xl border border-border bg-ink p-7 transition hover:border-mint"
                >
                  ...
                  <a
                    href={item.route}
                    className="mt-auto pt-7 inline-flex items-center gap-2 font-bold text-mint"
                  >
                    {item.cta}{" "}
          ...
          <div className="mt-10 grid gap-5 md:grid-cols-4">
            {[
              ...
              [
                "02",
                "Baseline",
                "Separate measured data from assumptions and missing information.",
              ],
              [
                "03",
                "Intervene",
                "Apply the smallest useful correction that addresses the constraint.",
              ],
      ...
      <section>
        <div className="mx-auto max-w-5xl px-6 py-20 text-center md:py-28">
          ...
          <h2 className="mt-4 text-balance font-display text-4xl font-black text-ice md:text-6xl">
            The operating model behind the website is the product.
          </h2>
          <p className="mx-auto mt-6 max-w-3xl text-lg leading-relaxed text-muted-foreground">
            INKSIGHTS is building a proprietary dataset covering the conditions that shape
            tattoo-studio performance: visibility, enquiries, bookings, capacity, cancellations,
            transaction value, retention and market signals.
          </p>
        </div>
      </section>
      <IntelligenceMethodology />
      <CtaSection
        title="Start with your studio's actual constraint."
        description="Run the free Studio Growth Check, get the first-pass opportunity estimate and decide whether a paid Full INKSIGHTS Audit is justified."
      />
    </PublicShell>
  );
}
