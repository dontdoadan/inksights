  ...
  head: ({ params }) => {
    ...
    return {
      meta: [
        ...
        {
          name: "description",
          content: offer?.summary || "INKSIGHTS tattoo studio growth solution.",
        },
...
function OfferPage() {
    ...
    return (
      <PublicShell>
        <PageHero
          ...
          title="This solution is not currently published."
          description="Return to the complete INKSIGHTS offer catalogue."
          compact
        >
  ...
  return (
    <PublicShell>
      ...
      <div className="border-b border-border bg-ink px-6 py-6">
        <p className="mx-auto max-w-7xl text-sm leading-relaxed text-muted-foreground">
          Current stage: the diagnostic engine, canonical data model, growth playbooks and
          action/result tracking are in active build. Service scope is agreed separately; a
          published offer does not imply live integrations or validated outcomes. Our priority is
          validating the full loop on one real business before widening integrations and
          acquisition.
        </p>
      </div>
      ...
      {search.checkout === "success" && (
        <div className="border-b border-mint/30 bg-mint/10 px-6 py-4 text-center text-sm font-medium text-mint">
          Payment successful. Thank you — INKSIGHTS will contact you within one working day to begin
          intake.
        </div>
      )}
      ...
      <section className="border-b border-border bg-ink">
        <div className="mx-auto grid max-w-7xl gap-6 px-6 py-10 md:grid-cols-2 lg:grid-cols-4">
          ...
          <Info
            label="Growth levers"
            value={offer.growthLevers.map((lever) => growthLeverLabels[lever]).join(" · ")}
          />
          ...
          <div className="grid gap-12 lg:grid-cols-2">
            <div>
              <SectionHeading
                ...
                title="A defined output, not an open-ended promise."
              />
              <ul className="mt-8 space-y-4">
                {offer.deliverables.map((item) => (
                  <li
                    key={item}
                    className="flex gap-3 rounded-xl border border-border bg-ink p-4 text-muted-foreground"
                  >
      ...
      <section className="border-y border-border bg-ink">
        <div className="mx-auto max-w-7xl px-6 py-20 md:py-28">
          <SectionHeading
            ...
            title="How the work moves from diagnosis to verification."
          />
          <div className="mt-10 grid gap-5 lg:grid-cols-4">
            {offer.process.map((step, index) => (
              <div key={step.title} className="rounded-2xl border border-border bg-ink-deep p-6">
                <div className="font-display text-4xl font-black text-mint/35">
      ...
      <section>
        <div className="mx-auto grid max-w-7xl gap-8 px-6 py-20 lg:grid-cols-2 md:py-28">
          <div className="rounded-2xl border border-mint/35 bg-mint/5 p-7">
            <h2 className="flex items-center gap-3 font-display text-2xl font-black text-ice">
              <CheckCircle2 className="h-6 w-6 text-mint" />
              What the studio must provide
            </h2>
      ...
      <section className="border-t border-border bg-ink">
        <div className="mx-auto max-w-5xl px-6 py-20 md:py-28">
          <SectionHeading
            ...
            title="Scope, billing and limitations."
          />
          <div className="mt-8 space-y-3">
            {offer.faqs.map(([question, answer]) => (
              <details
                key={question}
                className="rounded-2xl border border-border bg-ink-deep p-5 open:border-mint/50"
              >
                <summary className="cursor-pointer list-none font-display text-lg font-bold text-ice">
                  {question}
                </summary>
...
function Info({ label, value, icon }: { label: string; value: string; icon?: ReactNode }) {
  return (
    <div className="rounded-2xl border border-border bg-ink-deep p-5">
      <div className="flex items-center gap-2 text-xs font-bold uppercase tracking-[0.15em] text-mint">
        {icon}
        {label}
      </div>
