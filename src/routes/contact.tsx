...
function ContactPage() {
  ...
  return (
    <PublicShell>
      <PageHero eyebrow="Contact INKSIGHTS" title={<>Ask a specific question or start with the diagnosis.</>} description={<>Use the form for support, billing, offer scope, partnerships, technical issues or general enquiries. Studio owners seeking a recommendation should normally complete the free Revenue Audit first.</>} />
      <section>
        <div className="mx-auto grid max-w-7xl gap-10 px-6 py-16 md:py-24 lg:grid-cols-[.8fr_1.2fr]">
          <div className="space-y-5">
            ...
            <div className="rounded-2xl border border-border bg-ink p-6"><MessageSquareText className="h-7 w-7 text-mint" /><h2 className="mt-5 font-display text-2xl font-black text-ice">Studio recommendation</h2><p className="mt-3 text-sm leading-relaxed text-muted-foreground">Start with the free Revenue Audit for a structured commercial diagnosis.</p><Link to="/studio-growth-check" className="mt-5 inline-flex rounded-full bg-mint px-5 py-3 text-sm font-bold text-ink-deep">Start the free audit</Link></div>
            ...
            {status === "done" ? (
              ...
              <form onSubmit={submit} className="space-y-5">
                ...
                <div className="grid gap-5 sm:grid-cols-2">
                  ...
                  <Field label="Topic" required><select name="topic" required className="form-control"><option value="">Select a topic</option><option value="existing-client-support">Existing client support</option><option value="billing-cancellation">Billing, subscription or cancellation</option><option value="growth-audit">Revenue Audit or recommendation</option><option value="partnership">Partnership or case study</option><option value="website-support">Website or technical issue</option><option value="privacy-data-request">Privacy or data request</option><option value="other">Other</option></select></Field>
                </div>
                <Field label="Message" required><textarea name="message" required minLength={10} maxLength={10000} rows={7} className="form-control resize-y" placeholder="Include what happened and the outcome you need. Do not include passwords, card details, API keys or verification codes." /></Field>
                <div className="hidden" aria-hidden="true"><label>Company URL<input name="company_url" tabIndex={-1} autoComplete="off" /></label></div>
                <label className="flex items-start gap-3 rounded-xl border border-border bg-ink-deep p-4 text-sm leading-relaxed text-muted-foreground"><input name="consent" type="checkbox" required className="mt-1 h-4 w-4" /><span>I agree that INKSIGHTS may store and use these details to respond to this enquiry. <span className="text-mint">Required.</span></span></label>
