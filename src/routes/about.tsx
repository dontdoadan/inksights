...
const principles = [
  {
    ...
    description: "Demonstrations are labelled as demonstrations. Unknown values stay unknown. Client outcomes are not invented to make an early-stage business appear larger.",
  },
  {
    icon: Workflow,
    title: "Fix the workflow before adding software",
    description: "A new platform cannot repair unclear ownership, weak policies or an undefined booking journey. Map the process first, then select the tool.",
  },
  {
    icon: ShieldCheck,
    title: "Automate with boundaries",
    description: "Automations require consent, human escalation, dry-run validation and a way to stop. High-risk or unusual situations should not be hidden inside a workflow.",
  },
  {
    icon: CheckCircle2,
    title: "Use the smallest useful offer",
    description: "A studio with one visibility problem should not be sold a large ongoing engagement. Scope should expand only when evidence and capacity justify it.",
  },
] as const;
    ...
    <PublicShell>
      ...
      <section className="border-y border-border bg-ink">
        <div className="mx-auto max-w-7xl px-6 py-20 md:py-28">
          <SectionHeading eyebrow="Operating principles" title="How early trust should be earned." />
          <div className="mt-10 grid gap-5 md:grid-cols-2">
            {principles.map((principle) => {
              const Icon = principle.icon;
              return (
                <div key={principle.title} className="rounded-2xl border border-border bg-ink-deep p-7">
                  <Icon className="h-7 w-7 text-mint" />
                  <h2 className="mt-5 font-display text-2xl font-black text-ice">{principle.title}</h2>
