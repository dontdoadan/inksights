...
export const Route = createRootRouteWithContext<{ queryClient: QueryClient }>()({
  head: () => ({ meta: [
    ...
    { property: "og:image", content: "https://getinksights.co.uk/og/inksight-growth-systems.png" },
    { name: "twitter:card", content: "summary_large_image" },
    { name: "twitter:image", content: "https://getinksights.co.uk/og/inksight-growth-systems.png" },
  ], links: [
...
function RootShell({ children }: { children: ReactNode }) { return <html lang="en-GB"><head><HeadContent /></head><body>{children}<CookieConsent /><Toaster position="top-center" richColors /><Scripts /></body></html>; }
function RootComponent() {
  const { queryClient } = Route.useRouteContext(); const router = useRouter(); const pathname = useRouterState({ select: (state) => state.location.pathname });
  useEffect(() => { if (typeof window === "undefined") return; let cancelled = false; import("@/integrations/supabase/client").then(({ supabase }) => { if (cancelled) return; const { data: sub } = supabase.auth.onAuthStateChange((event) => { if (event !== "SIGNED_IN" && event !== "SIGNED_OUT" && event !== "USER_UPDATED") return; router.invalidate(); if (event !== "SIGNED_OUT") queryClient.invalidateQueries(); }); (window as unknown as { __inksightAuthSub?: { unsubscribe: () => void } }).__inksightAuthSub?.unsubscribe(); (window as unknown as { __inksightAuthSub?: { unsubscribe: () => void } }).__inksightAuthSub = sub.subscription; }); return () => { cancelled = true; }; }, [queryClient, router]);
  useEffect(() => { if (typeof window === "undefined" || !readConsent()?.marketing) return; trackMetaPageView(pathname); }, [pathname]);
