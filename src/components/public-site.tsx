      ...
      <header className="site-header sticky top-0 z-50 border-b border-border/50 bg-ink-deep/90 backdrop-blur-xl">
        <div className="mx-auto flex max-w-7xl items-center justify-between px-5 py-4 md:px-6">
          ...
          <nav className="hidden items-center gap-7 text-sm font-semibold text-muted-foreground lg:flex" aria-label="Primary navigation">
            ...
            <a href="/studio-growth-check" className="shine-button rounded-full bg-mint px-5 py-2.5 font-bold text-ink-deep transition hover:bg-mint-soft">Free Studio Audit</a>
          </nav>
          <button type="button" aria-label={open ? "Close navigation" : "Open navigation"} aria-expanded={open} onClick={() => setOpen((value) => !value)} className="flex h-11 w-11 items-center justify-center rounded-xl border border-border text-ice transition hover:border-mint hover:text-mint lg:hidden">
            {open ? <X className="h-5 w-5" /> : <Menu className="h-5 w-5" />}
          </button>
        </div>
        {open ? <nav className="border-t border-border bg-ink px-5 py-5 lg:hidden" aria-label="Mobile navigation">
          <div className="mx-auto grid max-w-7xl gap-2">
            {navItems.map(([href, label]) => <a key={href} href={href} onClick={() => setOpen(false)} className="rounded-xl px-4 py-3 font-semibold text-ice transition hover:bg-ink-elev hover:text-mint">{label}</a>)}
            <a href="/studio-growth-check" onClick={() => setOpen(false)} className="shine-button mt-2 rounded-xl bg-mint px-4 py-3 text-center font-bold text-ink-deep">Run the free Studio Audit</a>
          </div>
        </nav> : null}
      </header>
    </>
...
export function SiteFooter() {
  const groups = [
    { title: "Diagnose", links: [["/studio-growth-check", "Free Revenue Audit"], ["/tattoo-studio-visibility-scorecard", "Visibility Scorecard"], ["/growth-model", "Studio Growth Model"]] },
    { title: "Fix", links: [["/offers", "All solutions"], ["/offers/72-hour-visibility-fix", "Visibility Fix"], ["/offers/visibility-watch", "Visibility Watch"], ["/offers/booking-retention-engine", "Booking & Retention"]] },
    { title: "Company", links: [["/case-studies", "Proof"], ["/about", "About INKSIGHTS"], ["/contact", "Contact"], ["/privacy", "Privacy"], ["/terms", "Terms"]] },
  ];
  return <footer className="border-t border-border bg-ink">
    <div className="mx-auto max-w-7xl px-6 py-14">
      <div className="grid gap-10 lg:grid-cols-[1.25fr_2fr]">
        <div><Logo /><p className="mt-5 max-w-sm text-sm leading-relaxed text-muted-foreground">Growth intelligence for UK tattoo studios. Diagnose the constraint, fix what matters and measure what changed.</p><a href="mailto:dontdoadan@icloud.com" className="mt-5 inline-block text-sm font-semibold text-mint hover:text-mint-soft">dontdoadan@icloud.com</a></div>
        <div className="grid gap-8 sm:grid-cols-3">{groups.map((group) => <div key={group.title}><h2 className="text-xs font-bold uppercase tracking-[0.16em] text-ice">{group.title}</h2><ul className="mt-4 space-y-3 text-sm text-muted-foreground">{group.links.map(([href, label]) => <li key={href}><a href={href} className="transition hover:text-mint">{label}</a></li>)}</ul></div>)}</div>
      </div>
      <div className="mt-12 flex flex-col gap-4 border-t border-border pt-6 text-xs text-muted-foreground sm:flex-row sm:items-center sm:justify-between">
        ...
        <button type="button" onClick={() => window.dispatchEvent(new Event("inksight:open-consent"))} className="text-left transition hover:text-mint">Change cookie preferences</button>
      </div>
    </div>
  </footer>;
}

export function PublicShell({ children }: { children: ReactNode }) { return <div className="min-h-screen overflow-x-hidden bg-ink-deep text-foreground"><ScrollProgress /><SiteHeader /><main id="main-content">{children}</main><SiteFooter /></div>; }
...
export function PageHero({ eyebrow, title, description, children, compact = false }: { eyebrow: string; title: ReactNode; description: ReactNode; children?: ReactNode; compact?: boolean }) {
  return <section className="hero-ambient relative overflow-hidden border-b border-border grid-bg"><div className="ambient-orb ambient-orb-one" aria-hidden="true" /><div className="ambient-orb ambient-orb-two" aria-hidden="true" /><div className="pointer-events-none absolute inset-0 bg-gradient-to-b from-transparent via-ink-deep/55 to-ink-deep" /><div className={`relative mx-auto max-w-7xl px-6 ${compact ? "py-16 md:py-20" : "py-20 md:py-28"}`}><p className="text-xs font-bold uppercase tracking-[0.2em] text-mint">{eyebrow}</p><h1 className="mt-5 max-w-5xl text-balance font-display text-4xl font-black leading-[1.02] tracking-tight text-ice sm:text-5xl md:text-7xl">{title}</h1><div className="mt-6 max-w-3xl text-lg leading-relaxed text-muted-foreground md:text-xl">{description}</div>{children ? <div className="mt-9 flex flex-wrap gap-3">{children}</div> : null}</div></section>;
}
...
export function CtaSection({ eyebrow = "Start with the diagnosis", title = "Find the constraint before buying another tool.", description = "The free Studio Growth Check identifies the strongest commercial pressure and routes the studio to the most useful next step." }: { eyebrow?: string; title?: string; description?: string }) { return <section className="relative overflow-hidden border-y border-border bg-ink"><div className="ambient-orb ambient-orb-one" aria-hidden="true" /><div className="relative mx-auto max-w-5xl px-6 py-16 text-center md:py-24"><p className="text-xs font-bold uppercase tracking-[0.18em] text-mint">{eyebrow}</p><h2 className="mt-4 text-balance font-display text-4xl font-black text-ice md:text-6xl">{title}</h2><p className="mx-auto mt-5 max-w-2xl text-lg leading-relaxed text-muted-foreground">{description}</p><div className="mt-8 flex justify-center"><PrimaryButton href="/studio-growth-check">Start the free Studio Audit</PrimaryButton></div></div></section>; }
