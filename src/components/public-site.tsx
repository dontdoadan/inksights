...
export function SiteHeader() {
  ...
  return (
    <>
      <a
        href="#main-content"
        className="sr-only z-[100] rounded-md bg-mint px-4 py-2 font-bold text-ink-deep focus:not-sr-only focus:fixed focus:left-4 focus:top-4"
      >
        Skip to content
      </a>
        ...
        <div className="mx-auto flex max-w-7xl items-center justify-between px-5 py-4 md:px-6">
          ...
          >
            {navItems.map(([href, label]) => (
              <a key={href} href={href} className="nav-link transition hover:text-mint">
                {label}
              </a>
            ))}
            <a
              href="/studio-growth-check"
              className="shine-button rounded-full bg-mint px-5 py-2.5 font-bold text-ink-deep transition hover:bg-mint-soft"
            >
              Free Studio Growth Check
            </a>
          </nav>
          <button
            type="button"
            ...
            <div className="mx-auto grid max-w-7xl gap-2">
              {navItems.map(([href, label]) => (
                <a
                  ...
                  className="rounded-xl px-4 py-3 font-semibold text-ice transition hover:bg-ink-elev hover:text-mint"
                >
                  {label}
                </a>
              ))}
              <a
                href="/studio-growth-check"
                onClick={() => setOpen(false)}
                className="shine-button mt-2 rounded-xl bg-mint px-4 py-3 text-center font-bold text-ink-deep"
              >
  ...
  const groups = [
    {
      ...
      links: [
        ...
        ["/offers/revenue-audit", "Full INKSIGHTS Audit"],
        ...
        <div className="grid gap-10 lg:grid-cols-[1.25fr_2fr]">
          <div>
            ...
            <p className="mt-5 max-w-sm text-sm leading-relaxed text-muted-foreground">
              Growth intelligence, diagnostics and commercial systems designed specifically for UK
              tattoo studios.
            </p>
            <a
              href="mailto:dontdoadan@icloud.com"
              className="mt-5 inline-block text-sm font-semibold text-mint hover:text-mint-soft"
            >
          ...
          <div className="grid gap-8 sm:grid-cols-3">
            {groups.map((group) => (
              <div key={group.title}>
                <h2 className="text-xs font-bold uppercase tracking-[0.16em] text-ice">
                  {group.title}
                </h2>
...
export function PageHero({
  ...
  return (
    <section className="hero-ambient relative overflow-hidden border-b border-border grid-bg">
      ...
      <div
        className={`relative mx-auto max-w-7xl px-6 ${compact ? "py-16 md:py-20" : "py-20 md:py-28"}`}
      >
        ...
        <div className="mt-6 max-w-3xl text-lg leading-relaxed text-muted-foreground md:text-xl">
          {description}
        </div>
...
export function PrimaryButton({ href, children }: { href: string; children: ReactNode }) {
  return (
    <a
      href={href}
      className="shine-button group inline-flex min-h-12 items-center justify-center gap-2 rounded-full bg-mint px-6 py-3 font-bold text-ink-deep transition hover:bg-mint-soft"
    >
      {children}
      <ArrowRight
...
export function SecondaryButton({ href, children }: { href: string; children: ReactNode }) {
  return (
    <a
      href={href}
      className="outline-button inline-flex min-h-12 items-center justify-center rounded-full border border-border px-6 py-3 font-bold text-ice transition hover:border-mint hover:text-mint"
    >
      {children}
    </a>
  );
...
export function SectionHeading({
  ...
  return (
    <div className="max-w-3xl">
      ...
      {description ? (
        <div className="mt-4 text-base leading-relaxed text-muted-foreground md:text-lg">
          {description}
        </div>
      ) : null}
    </div>
...
export function Card({ children, className = "" }: { children: ReactNode; className?: string }) {
  return (
    <div className={`interactive-card rounded-2xl border border-border bg-ink p-6 ${className}`}>
      {children}
    </div>
  );
}
export function CtaSection({
  ...
  description = "The free Studio Growth Check provides a first-pass opportunity estimate from your answers. Use it to decide whether a paid Full INKSIGHTS Audit is worth discussing.",
}: {
  eyebrow?: string;
  title?: string;
  description?: string;
...
export function RevenueLeakageMap() {
  ...
  return (
    <div className="rounded-3xl border border-mint/25 bg-ink-deep p-5 shadow-2xl shadow-black/20 md:p-7">
      <div className="flex items-center justify-between gap-4 border-b border-border pb-5">
        <div>
          <p className="text-xs font-bold uppercase tracking-[.16em] text-mint">
            Revenue leakage map
          </p>
          <h3 className="mt-2 font-display text-2xl font-black text-ice">
            Where does momentum disappear?
          </h3>
        </div>
        <div className="hidden h-10 px-3 items-center justify-center rounded-xl border border-mint/30 bg-mint/10 font-mono text-xs text-mint sm:flex">
          MODEL
        </div>
      </div>
      <div className="mt-7 grid gap-3 md:grid-cols-5">
        {steps.map(([number, label, text], index) => (
          <div key={number} className="relative rounded-2xl border border-border bg-ink p-5">
            ...
            {index < steps.length - 1 ? (
              <span
                className="pointer-events-none absolute -right-2.5 top-1/2 hidden h-px w-5 bg-mint/50 md:block"
                aria-hidden="true"
              />
            ) : null}
          </div>
      ...
      <div className="mt-5 rounded-2xl border border-amber-300/25 bg-amber-300/5 p-4 text-sm leading-relaxed text-muted-foreground">
        <strong className="text-ice">INKSIGHTS principle:</strong> more traffic is not automatically
        the answer. Find the first material point where demand, time or value is being lost.
      </div>
    </div>
  );
}
...
export function ArticleLayout({ children, aside }: { children: ReactNode; aside?: ReactNode }) {
  return (
    <section className="bg-ink-deep">
      <div
        className={`mx-auto grid max-w-7xl gap-10 px-6 py-14 md:py-20 ${aside ? "lg:grid-cols-[minmax(0,1fr)_320px]" : "max-w-4xl"}`}
      >
...
export function SourceList({
  ...
  return (
    <section aria-labelledby="sources-heading" className="mt-12 border-t border-border pt-8">
      <h2 id="sources-heading" className="font-display text-2xl font-black text-ice">
        ...
        {sources.map((source) => (
          <li key={source.href}>
            <a
              ...
              target="_blank"
              rel="noreferrer"
              className="font-semibold text-mint hover:text-mint-soft"
            >
              {source.label}
            </a>
