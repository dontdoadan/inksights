      ...
      <div className="mx-auto max-w-7xl px-6">
        <div className="flex flex-col gap-8 lg:flex-row lg:items-end lg:justify-between">
          <div className="max-w-3xl">
            <p className="text-xs font-bold uppercase tracking-[0.2em] text-mint">
              Demonstration · planned intelligence layer
            </p>
            ...
            <p className="mt-5 max-w-2xl text-lg leading-relaxed text-muted-foreground">
              Explore a demonstration of studio discovery and comparison. All records, scores and
              artist counts below are illustrative; this map is not connected to a live studio
              dataset.
            </p>
          </div>
          <div className="grid grid-cols-3 gap-2 text-right sm:gap-4">
            <Metric value={String(demoStudios.length)} label="demo records" />
            <Metric value="Demo" label="illustrative scores" />
            <Metric value="Planned" label="dataset connection" />
          </div>
        </div>
        ...
        <div className="mt-10 grid overflow-hidden rounded-3xl border border-border bg-ink shadow-2xl shadow-black/20 lg:grid-cols-[1.4fr_.6fr]">
          <div className="relative min-h-[620px] overflow-hidden border-b border-border bg-[#071017] lg:border-b-0 lg:border-r">
            ...
            <div className="absolute left-6 right-6 top-6 z-10 flex flex-wrap gap-2">
              <label className="flex min-w-0 w-full sm:w-auto sm:min-w-[260px] items-center gap-2 rounded-xl border border-border bg-ink/90 px-3 py-2 backdrop-blur">
                <Search className="h-4 w-4 text-muted-foreground" />
                <input
                  value={query}
                  onChange={(e) => setQuery(e.target.value)}
                  aria-label="Search demonstration studios"
              ...
              <div className="flex h-full flex-col p-6">
                ...
                <div className="mt-5">
                  ...
                  <div className="mt-8 grid grid-cols-2 gap-3">
                    <DataPoint label="Illustrative score" value={`${selected.score}/100`} />
                    <DataPoint label="Illustrative artists" value={String(selected.artists)} />
                  </div>
                  <div className="mt-8 rounded-2xl border border-border bg-ink-deep p-5">
                    ...
                    <p className="mt-3 text-sm leading-relaxed text-muted-foreground">
                      Planned: connect verified studio observations after the canonical model and
                      full diagnostic loop are validated. This profile is a demonstration, not a
                      real studio assessment.
                    </p>
                  </div>
                </div>
              </div>
