...
const demoStudios: Studio[] = [
  { name: "London Studio Cluster", city: "London", region: "London", postcode: "SW1", tier: "stronger", artists: 8, score: 91, x: 66, y: 78 },
  { name: "Brighton Studio Cluster", city: "Brighton", region: "South East", postcode: "BN1", tier: "stronger", artists: 6, score: 84, x: 67, y: 86 },
  { name: "Birmingham Studio Cluster", city: "Birmingham", region: "West Midlands", postcode: "B1", tier: "standard", artists: 5, score: 72, x: 47, y: 56 },
  { name: "Manchester Studio Cluster", city: "Manchester", region: "North West", postcode: "M1", tier: "stronger", artists: 7, score: 88, x: 45, y: 35 },
  { name: "Leeds Studio Cluster", city: "Leeds", region: "Yorkshire", postcode: "LS1", tier: "standard", artists: 5, score: 69, x: 53, y: 32 },
  { name: "Liverpool Studio Cluster", city: "Liverpool", region: "North West", postcode: "L1", tier: "review", artists: 3, score: 54, x: 39, y: 39 },
  { name: "Bristol Studio Cluster", city: "Bristol", region: "South West", postcode: "BS1", tier: "standard", artists: 4, score: 67, x: 38, y: 69 },
  { name: "Edinburgh Studio Cluster", city: "Edinburgh", region: "Scotland", postcode: "EH1", tier: "stronger", artists: 6, score: 86, x: 47, y: 10 },
  { name: "Glasgow Studio Cluster", city: "Glasgow", region: "Scotland", postcode: "G1", tier: "standard", artists: 4, score: 70, x: 40, y: 13 },
  { name: "Cardiff Studio Cluster", city: "Cardiff", region: "Wales", postcode: "CF1", tier: "review", artists: 3, score: 58, x: 35, y: 67 },
  { name: "Nottingham Studio Cluster", city: "Nottingham", region: "East Midlands", postcode: "NG1", tier: "standard", artists: 4, score: 65, x: 54, y: 50 },
  { name: "Newcastle Studio Cluster", city: "Newcastle", region: "North East", postcode: "NE1", tier: "standard", artists: 3, score: 63, x: 61, y: 24 },
];
        ...
        <div className="flex flex-col gap-8 lg:flex-row lg:items-end lg:justify-between">
          <div className="max-w-3xl">
            ...
            <p className="mt-5 max-w-2xl text-lg leading-relaxed text-muted-foreground">
              An interactive intelligence layer for discovering studios, comparing signals and turning geography into a commercial decision surface.
            </p>
          </div>
        ...
        <div className="mt-10 grid overflow-hidden rounded-3xl border border-border bg-ink shadow-2xl shadow-black/20 lg:grid-cols-[1.4fr_.6fr]">
          <div className="relative min-h-[620px] overflow-hidden border-b border-border bg-[#071017] lg:border-b-0 lg:border-r">
            ...
            <div className="absolute left-6 top-6 z-10 flex flex-wrap gap-2">
              ...
              <button onClick={() => setTier(tier === "all" ? "stronger" : tier === "stronger" ? "review" : "all")} className="inline-flex items-center gap-2 rounded-xl border border-border bg-ink/90 px-4 py-2 text-sm font-bold text-ice backdrop-blur hover:border-mint">
            ...
            <div className="absolute inset-x-[10%] top-[15%] bottom-[8%]">
              <svg viewBox="0 0 500 700" className="h-full w-full" role="img" aria-label="Stylised interactive map of the United Kingdom">
                <path d="M247 30 278 54 286 94 272 126 291 158 280 191 299 219 282 247 293 279 275 308 286 343 270 377 278 414 256 448 269 483 247 520 227 508 214 475 193 449 176 414 151 388 131 351 105 327 113 291 97 258 113 229 105 193 124 164 145 147 151 111 175 88 182 54 206 42Z" fill="rgba(160,255,218,.055)" stroke="rgba(160,255,218,.32)" strokeWidth="2" />
                <path d="M83 338 105 327 125 344 121 370 100 383 77 368Z" fill="rgba(160,255,218,.045)" stroke="rgba(160,255,218,.22)" strokeWidth="2" />
                <path d="M205 510 224 527 211 548 188 544 181 525Z" fill="rgba(160,255,218,.045)" stroke="rgba(160,255,218,.22)" strokeWidth="2" />
              </svg>

              {filtered.map((studio) => (
                <button key={studio.name} onClick={() => setSelected(studio)} className="group absolute -translate-x-1/2 -translate-y-1/2" style={{ left: `${studio.x}%`, top: `${studio.y}%` }} aria-label={`Open ${studio.name}`}>
                  <span className={`block h-3 w-3 rounded-full border-2 border-ink shadow-[0_0_0_5px_rgba(160,255,218,.08)] transition group-hover:scale-150 ${studio.tier === "review" ? "bg-amber-300" : studio.tier === "stronger" ? "bg-mint" : "bg-sky-300"}`} />
                  <span className="pointer-events-none absolute left-1/2 top-5 hidden -translate-x-1/2 whitespace-nowrap rounded-lg border border-border bg-ink px-2 py-1 text-[10px] font-bold text-ice shadow-xl group-hover:block">{studio.city}</span>
                </button>
              ))}
            </div>
                ...
                <div className="flex-1 overflow-auto p-3">
                  {filtered.map((studio) => (
                    <button key={studio.name} onClick={() => setSelected(studio)} className="flex w-full items-center gap-3 rounded-xl p-3 text-left transition hover:bg-ink-elev">
                      <span className={`h-2.5 w-2.5 shrink-0 rounded-full ${studio.tier === "review" ? "bg-amber-300" : studio.tier === "stronger" ? "bg-mint" : "bg-sky-300"}`} />
