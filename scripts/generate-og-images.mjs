  ...
  for (const raw of text.toUpperCase()) {
    ...
    glyph.forEach((row, rowIndex) => {
      [...row].forEach((value, columnIndex) => {
        if (value === "1")
          rect(buffer, cursor + columnIndex * scale, y + rowIndex * scale, scale, scale, colour);
      });
    });
  ...
  if (kind === "growth") {
    for (let i = 0; i < 5; i += 1) {
      rect(
        ...
        28,
        135 + i * 42,
        i === 4 ? palette.mint : palette.raised,
      );
    }
    ...
    points.forEach((point, index) =>
      circle(
        ...
        point[1],
        14 + index * 2,
        index % 2 ? palette.mintSoft : palette.mint,
      ),
    );
  } else if (kind === "cost") {
    for (let i = 0; i < 4; i += 1) {
...
const images = [
  {
    file: "inksight-growth-systems.png",
    label: "GROWTH SYSTEMS",
    lines: ["TURN VISIBILITY INTO", "BOOKED CLIENTS"],
    pattern: "growth",
  },
  {
    file: "tattoo-pain-reality-check.png",
    label: "CLIENT EDUCATION",
    lines: ["TATTOO PAIN CHART", "REALITY CHECK"],
    pattern: "pain",
  },
  {
    file: "full-sleeve-cost-uk.png",
    label: "UK PRICING GUIDE",
    lines: ["WHAT DOES A FULL", "SLEEVE COST"],
    pattern: "cost",
  },
  {
    file: "grey-line-healing.png",
    label: "HEALING GUIDE",
    lines: ["GREY LINE HEALING", "WEEK BY WEEK"],
    pattern: "healing",
  },
];
