...
alter table public.audit_recommendations
  add column if not exists phase text generated always as (
    case
      when sequence > 20 then '61-90'
      when sequence > 10 then '31-60'
      else '0-30'
    end
  ) stored;
...
declare
  v_studio_id uuid;
  v_source_id uuid;
  v_source_rows integer := coalesce(pg_catalog.jsonb_array_length(p_rows),0) + coalesce(pg_catalog.jsonb_array_length(p_rejected),0);
...
begin
  ...
  update public.audits set status='normalising', started_at=coalesce(started_at,pg_catalog.now()) where id=p_audit_id;
  ...
  return pg_catalog.jsonb_build_object(
    ...
    'parsed_rows',coalesce(pg_catalog.jsonb_array_length(p_rows),0),'rejected_rows',coalesce(pg_catalog.jsonb_array_length(p_rejected),0),
