...
with check (exists (select 1 from public.studio_members m where m.studio_id=intelligence_decisions.studio_id and m.user_id=(select auth.uid()) and m.active and m.role in ('owner','admin','member')));
...
using (exists (select 1 from public.studio_members m where m.studio_id=intelligence_decisions.studio_id and m.user_id=(select auth.uid()) and m.active and m.role in ('owner','admin','member')))
with check (exists (select 1 from public.studio_members m where m.studio_id=intelligence_decisions.studio_id and m.user_id=(select auth.uid()) and m.active and m.role in ('owner','admin','member')));
...
on public.intelligence_interventions for insert to authenticated
with check (exists (select 1 from public.studio_members m where m.studio_id=intelligence_interventions.studio_id and m.user_id=(select auth.uid()) and m.active and m.role in ('owner','admin','member')));
...
on public.intelligence_interventions for update to authenticated
using (exists (select 1 from public.studio_members m where m.studio_id=intelligence_interventions.studio_id and m.user_id=(select auth.uid()) and m.active and m.role in ('owner','admin','member')))
with check (exists (select 1 from public.studio_members m where m.studio_id=intelligence_interventions.studio_id and m.user_id=(select auth.uid()) and m.active and m.role in ('owner','admin','member')));
...
declare
  service_tables text[] := array[
    'intelligence_diagnostic_questions','intelligence_diagnostic_templates','intelligence_source_registry','intelligence_taxonomy','orders','public_contact_requests','public_endpoint_rate_limits','revenue_audits','studio_aliases','studio_candidates','studio_change_events','studio_identity_matches','studio_source_observations','studio_sources','studio_verification_events','visibility_provider_configs'
  ];
  t text;
begin
...
-- 3) Realtime hardening: keep Supabase-managed realtime.messages policies intact,
-- and move the database broadcast trigger helper out of the exposed public schema.
...
set search_path = ''
as $$
declare studio_uuid uuid;
begin
  studio_uuid := coalesce(new.studio_id, old.studio_id);
  perform realtime.broadcast_changes('studio:' || studio_uuid::text, tg_op, tg_op, tg_table_name, tg_table_schema, new, old);
  return null;
end;
$$;
...
create trigger studio_dashboard_opportunities_broadcast after insert or update or delete on public.visibility_opportunities for each row execute function private.broadcast_studio_dashboard_changes();
create trigger studio_dashboard_pipeline_runs_broadcast after insert or update or delete on public.visibility_pipeline_runs for each row execute function private.broadcast_studio_dashboard_changes();
create trigger studio_dashboard_report_runs_broadcast after insert or update or delete on public.visibility_report_runs for each row execute function private.broadcast_studio_dashboard_changes();

-- 4) Private report storage. Backend writes; authenticated active studio members
-- can read only objects rooted at their own studio UUID.
insert into storage.buckets (id,name,public,file_size_limit,allowed_mime_types)
values ('studio-reports','studio-reports',false,26214400,array['application/pdf','application/json']::text[])
...
using (bucket_id='studio-reports' and exists (select 1 from public.studio_members sm where sm.user_id=(select auth.uid()) and sm.active and (storage.foldername(name))[1]=sm.studio_id::text));

-- 5) PostGIS: remove browser-level direct table access without enabling RLS on
-- extension internals, which could interfere with PostGIS's own lookups.
revoke all on table public.spatial_ref_sys from anon, authenticated;
revoke select on table public.spatial_ref_sys from public;

-- 6) Cover the composite foreign key reported by the Performance Advisor.
