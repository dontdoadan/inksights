...
-- Classification is mandatory so modelled values cannot silently become observed facts.
...
create table if not exists public.intelligence_evidence (
  ...
  source_ref text,
  metric_value_id uuid references public.intelligence_metric_values(id) on delete set null,
  claim text,
...
create table if not exists public.intelligence_findings (
  ...
  confidence numeric check (confidence is null or confidence between 0 and 1),
  status text not null default 'open' check (status in ('draft','open','validated','dismissed','resolved')),
...
create table if not exists public.intelligence_diagnoses (
  ...
  confidence numeric check (confidence is null or confidence between 0 and 1),
  status text not null default 'active' check (status in ('draft','active','rejected','resolved')),
  created_at timestamptz not null default now(),
  updated_at timestamptz not null default now()
);
...
create table if not exists public.intelligence_recommendations (
  id uuid primary key default gen_random_uuid(),
  studio_id uuid not null references public.visibility_studios(id) on delete cascade,
  diagnosis_id uuid not null references public.intelligence_diagnoses(id) on delete cascade,
  opportunity_id uuid references public.visibility_opportunities(id) on delete set null,
  title text not null,
  ...
  confidence numeric check (confidence is null or confidence between 0 and 1),
  evidence_ids uuid[] not null default '{}',
  status text not null default 'proposed' check (status in ('draft','proposed','accepted','rejected','superseded','completed')),
  created_at timestamptz not null default now(),
  updated_at timestamptz not null default now()
);
...
create table if not exists public.intelligence_outcomes (
  id uuid primary key default gen_random_uuid(),
  studio_id uuid not null references public.visibility_studios(id) on delete cascade,
  intervention_id uuid not null references public.intelligence_interventions(id) on delete cascade,
  metric_definition_id uuid references public.intelligence_metric_definitions(id) on delete set null,
  ...
  source_type text not null,
  source_ref text,
  classification text not null default 'observed' check (classification in ('observed','derived','modelled','inferred','evidence_backed')),
  confidence numeric check (confidence is null or confidence between 0 and 1),
...
create table if not exists public.intelligence_attributions (
  ...
  studio_id uuid not null references public.visibility_studios(id) on delete cascade,
  intervention_id uuid not null references public.intelligence_interventions(id) on delete cascade,
  outcome_id uuid not null references public.intelligence_outcomes(id) on delete cascade,
  ...
  attribution_confidence numeric check (attribution_confidence is null or attribution_confidence between 0 and 1),
  attributed_value numeric,
  attributed_value_pence bigint,
...
create table if not exists public.intelligence_learning (
  ...
  attribution_id uuid references public.intelligence_attributions(id) on delete set null,
  hypothesis text not null,
  result text not null,
  learning_type text not null,
  confidence numeric check (confidence is null or confidence between 0 and 1),
  applies_to jsonb not null default '{}'::jsonb,
  recommendation_adjustment jsonb not null default '{}'::jsonb,
  created_at timestamptz not null default now()
);
...
insert into public.intelligence_metric_definitions (key,name,description,unit,classification,version,formula)
values
