...
begin
  ...
  if v_source_id is not null then
    v_existing := true;
    return jsonb_build_object('source_id', v_source_id, 'idempotent', true);
  end if;

  insert into public.audit_runs(audit_id, engine_key, status, input_summary)
  values (p_audit_id, 'transaction_ingest', 'started', jsonb_build_object('source_type',p_source_type,'storage_path',p_storage_path))
  ...
  select distinct
    ...
    c.id,
    row_data->>'rawClient',
    row_data->>'normalisedClient',
    'normalised',
    'MEDIUM'
  from jsonb_array_elements(p_rows) row_data
  join public.clients c
    on c.studio_id = v_studio_id and c.canonical_label = row_data->>'normalisedClient'
  where coalesce(row_data->>'rawClient','') <> ''
  on conflict (studio_id, raw_label) do update
    set normalised_label = excluded.normalised_label,
  ...
  insert into public.transactions(
    studio_id, audit_source_id, client_id, client_alias_id, transaction_date, amount_pence,
    currency, raw_reference, duplicate_status, source_row_key
  )
  select
    v_studio_id,
    v_source_id,
  ...
  join public.clients c
    on c.studio_id = v_studio_id and c.canonical_label = row_data->>'normalisedClient'
  left join public.client_aliases ca
    on ca.studio_id = v_studio_id and ca.raw_label = row_data->>'rawClient'
  on conflict (audit_source_id, source_row_key) do nothing;

  update public.audit_runs
  set status = 'success', completed_at = now(), output_summary = jsonb_build_object(
    'source_rows', v_source_rows,
    'parsed_rows', coalesce(jsonb_array_length(p_rows),0),
    'rejected_rows', coalesce(jsonb_array_length(p_rejected),0),
    'potential_duplicate_rows', p_potential_duplicate_rows,
    'total_pence', p_total_pence,
    'client_aliases_created_or_updated', v_alias_count,
    'source_id', v_source_id
  )
  where id = v_run_id;

  return jsonb_build_object('source_id', v_source_id, 'run_id', v_run_id, 'idempotent', v_existing);
exception when others then
  if v_run_id is not null then
    update public.audit_runs set status='failed', completed_at=now(), error_code=sqlstate, error_message=sqlerrm where id=v_run_id;
  end if;
  raise;
end;
$$;
