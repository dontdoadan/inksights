...
Deno.serve(async (req: Request) => {
  ...
  if (!validated.ok) {
    return response({ error: validated.error }, 400, origin);
  }
  ...
  try {
    ...
    const fingerprint = await digest(
      `${ip}|contact|${Deno.env.get("RATE_LIMIT_SALT") || "inksights-contact-v1"}`,
    );
    ...
    const { name, email, studio_name, topic, message } = validated.value;
    const metadata = {
    ...
    return response(
      {
        ...
        message:
          "Your message has been recorded. INKSIGHTS will reply using the email provided.",
      },
      200,
      origin,
    );
