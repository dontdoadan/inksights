...
function decodeServiceAccount(raw: string): JsonRecord {
  try {
    ...
    return JSON.parse(atob(raw)) as JsonRecord;
  }
}
...
async function serviceAccountAccessToken(raw: string): Promise<string> {
  ...
  if (!response.ok)
    throw new Error(`Google service-account token exchange failed: ${JSON.stringify(payloadJson)}`);
...
async function refreshTokenAccessToken(): Promise<string> {
  ...
  if (!clientId || !clientSecret || !refreshToken)
    throw new Error("Google OAuth refresh-token credentials are incomplete.");
  const response = await fetch("https://oauth2.googleapis.com/token", {
    method: "POST",
    headers: { "Content-Type": "application/x-www-form-urlencoded" },
    body: new URLSearchParams({
  ...
  if (!response.ok)
    throw new Error(`Google refresh-token exchange failed: ${JSON.stringify(payload)}`);
...
async function googleAccessToken(): Promise<string> {
  ...
  return serviceAccount
    ? await serviceAccountAccessToken(serviceAccount)
...
async function fetchSearchAnalytics(
  ...
  for (let startRow = 0; startRow < 100_000; startRow += rowLimit) {
    ...
    if (!response.ok)
      throw new Error(
        `Search Console query failed (${response.status}): ${JSON.stringify(payload)}`,
      );
...
Deno.serve(async (req: Request) => {
  ...
  try {
    input = (await req.json()) as JsonRecord;
  } catch {
    input = {};
  }
  ...
  const credentialsPresent = Boolean(
    ...
    (Deno.env.get("GOOGLE_GSC_CLIENT_ID") &&
      ...
      Deno.env.get("GOOGLE_GSC_REFRESH_TOKEN")),
  );
  const { data: property, error: propertyError } = await supabase
    .from("seo_properties")
    .select("id,site_url,status,metadata")
    .eq("site_url", siteUrl)
    .maybeSingle();
  ...
  if (action === "health")
    return jsonResponse({
      module: "INKCARE Search Intelligence",
      property: property.site_url,
      propertyStatus: property.status,
      dryRun,
      credentialsPresent,
      ready: credentialsPresent && !dryRun,
      storage: "INKCARE Supabase",
    });

  const externalRunId = crypto.randomUUID();
  const { data: syncRun, error: runError } = await supabase
    .from("command_centre_sync_runs")
    .insert({
      provider: "google_search_console",
      run_type: "search_intelligence_sync",
      external_run_id: externalRunId,
      status: "started",
      details: { siteUrl, syncDays, dryRun },
    })
    .select("id")
    .single();
    ...
    await Promise.all([
      supabase
        ...
        .update({
          ...
          details: { siteUrl, syncDays, dryRun, code },
        })
        .eq("id", syncRun.id),
      supabase
        .from("command_centre_connector_health")
        .upsert(
          {
            provider: "google_search_console",
            account_reference: siteUrl,
            account_name: "INKCARE",
            connection_status: code === "GSC_AUTH_REQUIRED" ? "disconnected" : "critical",
            sync_status: "failed",
            last_attempt_at: new Date().toISOString(),
            response_time_ms: Date.now() - startedAt,
            error_count: 1,
            warning_count: code === "GSC_AUTH_REQUIRED" ? 1 : 0,
            last_error_code: code,
            last_error_message: message,
            catalogue_status: "ready",
            mapping_status: "ready",
            credentials_status: code === "GSC_AUTH_REQUIRED" ? "required" : "invalid",
            metadata: { module: "INKCARE Search Intelligence", dryRun },
      ...
      supabase
        .from("seo_properties")
        .update({
          status: code === "GSC_AUTH_REQUIRED" ? "awaiting_authorization" : "error",
          metadata: {
            ...((property.metadata as JsonRecord | null) ?? {}),
            credentials_required: code === "GSC_AUTH_REQUIRED",
            last_error: message,
          },
        })
        .eq("id", property.id),
  ...
  if (dryRun || !credentialsPresent)
    return await failSync(
      "GSC_AUTH_REQUIRED",
      dryRun
        ? "Search Console sync is deployed in DRY_RUN mode. Add credentials and set INKCARE_SEARCH_DRY_RUN=false."
        : "Google Search Console read-only credentials are missing.",
      409,
    );

  try {
    const accessToken = await googleAccessToken();
    const endDate = settledDate(3);
    const startDate = settledDate(syncDays + 2);
    ...
    for (const batch of chunk(pageRecords)) {
      const { error } = await supabase
        .from("seo_page_daily")
        .upsert(batch, { onConflict: "property_id,data_date,page_url,country,device,search_type" });
      if (error) throw new Error(`Page upsert failed: ${error.message}`);
      written += batch.length;
    }
    ...
    await supabase
      ...
      .update({
        status: "active",
        last_sync_at: now,
        last_successful_date: endDate,
        metadata: {
          ...((property.metadata as JsonRecord | null) ?? {}),
          domain: "getinkcare.co.uk",
          search_type: "web",
          credentials_required: false,
          latest_sync: {
            startDate,
            endDate,
            queryRows: queryRecords.length,
    ...
    if (opportunityError)
      throw new Error(`Opportunity refresh failed: ${opportunityError.message}`);
    ...
    await Promise.all([
      ...
      supabase
        ...
        .update({
          status: "success",
          records_read: queryRows.length + pageRows.length,
