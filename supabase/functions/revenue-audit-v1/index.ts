...
async function consumeRateLimit(req: Request) {
  ...
  const keyHash = await sha256(`revenue-audit-v1:${clientKey(req)}`);
  const response = await fetch(`${SB}/rest/v1/rpc/consume_public_endpoint_rate_limit`, {
    method: "POST",
    headers: { apikey: KEY, Authorization: `Bearer ${KEY}`, "Content-Type": "application/json" },
    body: JSON.stringify({ p_endpoint: "revenue-audit-v1", p_key_hash: keyHash, p_window_started_at: windowStartedAt, p_max_requests: 5 }),
  });
