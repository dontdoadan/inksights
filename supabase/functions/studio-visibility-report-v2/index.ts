...
const list = (value: unknown) =>
  Array.isArray(value)
    ? [...new Set(value.map((item) => clean(item, 80)).filter(Boolean))].slice(0, 12)
    : [];
...
async function snapshot(site: URL) {
  ...
  const [robotsAvailable, sitemapAvailable] = await Promise.all([
    probe("/robots.txt"),
    probe("/sitemap.xml"),
  ]);
...
Deno.serve(async (req) => {
  ...
  try {
    ...
    const studioName = clean(body.studio_name, 160);
    const contactName = clean(body.contact_name, 120);
    const email = clean(body.email, 254).toLowerCase();
    const area = clean(body.area, 120);
    const website = validatePublicUrl(body.website);
    const artists = Number(body.artist_count);
    const services = list(body.services);
    const styles = list(body.styles);
    const subjects = list(body.subjects);
    const bodyAreas = list(body.body_areas);
    const problemNeeds = list(body.problem_needs);
    const customerTypes = list(body.customer_types);
      ...
      return json(
        {
          ok: false,
          error:
            "Please provide a valid studio name, name, email, area and an artist count of 3 or more.",
        },
        400,
      );
    }
    ...
    if (!(await consumeRateLimit(sb, req)))
    ...
    const studioInsert = await sb
      .from("visibility_studios")
      .insert({
        studio_name: studioName,
        website_url: website.toString().replace(/\/$/, ""),
        town: area,
    ...
    const runInsert = await sb
      ...
      .insert({
        ...
        opportunity_summary: {},
        commercial_opportunity: { status: "not_calculated" },
        action_plan: {},
        methodology: { provider: "website_plus_provider_pipeline", search_data: "pending" },
        qa_checks: {},
    ...
    for (const [dimension, values] of [
      ...
      ["subject", subjects],
      ["body_area", bodyAreas],
      ["problem_need", problemNeeds],
      ["customer_type", customerTypes],
    ] as [string, string[]][]) {
      if (!values.length) continue;
      const capInsert = await sb.from("visibility_studio_capabilities").upsert(
        values.map((value) => ({
          studio_id: studioId,
          dimension,
          value,
          normalized_value: normalise(value),
          source_type: "studio_submitted",
          evidence: { report_run_id: runId },
        })),
        { onConflict: "studio_id,dimension,normalized_value" },
      );
      if (capInsert.error) throw new Error(`capability_${dimension}: ${capInsert.error.message}`);
    }

    const observationInsert = await sb.from("visibility_observations").insert({
      studio_id: studioId,
      observation_type: "website",
      entity_name: website.toString(),
      source_provider: "studio_website",
      source_reference: websiteData.final_url,
      observed_at: websiteData.observed_at,
      raw_data: { ...websiteData, kind: "homepage_snapshot" },
    });
    if (observationInsert.error)
      throw new Error(`website_observation: ${observationInsert.error.message}`);

    const ssuCount = await sb
    ...
    const update = await sb
      ...
      .update({
        status: "analysing",
        executive_summary:
          "This report contains a studio-specific search universe and a source-measured website snapshot. Search demand, rankings and revenue are measured only when a configured provider and first-party attribution source are available.",
        search_demand: { provider: "pending", generated_searches: ssuCount.count ?? 0 },
        current_visibility: { provider: "pending", rankings_measured: 0 },
        opportunity_summary: {
          status: "candidate_unverified",
          search_universe_count: ssuCount.count ?? 0,
        },
        commercial_opportunity: {
          status: "not_calculated",
          reason: "First-party attribution and conversion data are not connected.",
        },
        action_plan: { actions: [] },
        methodology: {
          provider: "website_plus_provider_pipeline",
          search_universe: "studio-specific generated candidates",
          revenue: "not calculated",
        },
        qa_checks: {
          website_http_status: websiteData.http_status,
          website_title_present: Boolean(websiteData.title),
          ssu_count: ssuCount.count ?? 0,
    ...
    EdgeRuntime.waitUntil(
      fetch(functionUrl, {
        ...
        body: JSON.stringify({ report_run_id: runId }),
      }).catch((error) => console.error("search-intelligence-trigger", error)),
    );
    ...
    return json({
      ...
      status: "analysing",
      provider: "website_plus_provider_pipeline",
      searchUniverseCount: ssuCount.count ?? 0,
      searchPipeline: "started",
      reportUrl: `https://getinksights.co.uk/studio-visibility-report?reportId=${runId}&token=${token}`,
    });
  } catch (error) {
    console.error(error);
    if (studioId) {
