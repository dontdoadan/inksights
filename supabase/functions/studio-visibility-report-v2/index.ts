...
function trustedOrigin(req: Request): boolean { const origin = req.headers.get("origin"); return Boolean(origin && ALLOWED_ORIGINS.has(origin)); }
function clientKey(req: Request): string { return req.headers.get("cf-connecting-ip")?.trim() || req.headers.get("x-forwarded-for")?.split(",")[0]?.trim() || "unknown"; }
...
async function consumeRateLimit(sb: ReturnType<typeof createClient>, req: Request): Promise<boolean> {
  const windowStartedAt = new Date(Math.floor(Date.now() / RATE_LIMIT_WINDOW_MS) * RATE_LIMIT_WINDOW_MS).toISOString();
  const keyHash = await sha256(`${clientKey(req)}|studio-visibility-report-v2`);
  const result = await sb.rpc("consume_public_endpoint_rate_limit", { p_endpoint: "studio-visibility-report-v2", p_key_hash: keyHash, p_window_started_at: windowStartedAt, p_max_requests: 5 });
...
async function readLimitedText(response: Response, maxBytes: number): Promise<string> {
  ...
  try { while (true) { const { done, value } = await reader.read(); if (done) break; total += value.byteLength; if (total > maxBytes) { await reader.cancel(); throw new Error("website_response_too_large"); } text += decoder.decode(value, { stream: true }); } return text + decoder.decode(); }
  finally { reader.releaseLock(); }
}
async function snapshot(site: URL) {
  let current = site; let response: Response | null = null;
  for (let redirectCount = 0; redirectCount <= MAX_REDIRECTS; redirectCount += 1) {
    await assertPublicNetworkTarget(current);
    response = await fetch(current, { redirect: "manual", headers: { "user-agent": "INKSIGHTS-Visibility-Check/1.3" }, signal: AbortSignal.timeout(10000) });
    if (![301, 302, 303, 307, 308].includes(response.status)) break;
    const location = response.headers.get("location"); await response.body?.cancel(); if (!location) throw new Error("website_redirect_invalid");
    current = validatePublicUrl(new URL(location, current).toString()); if (redirectCount === MAX_REDIRECTS) throw new Error("website_redirect_limit");
  }
  if (!response) throw new Error("website_fetch_failed");
  const observedAt = new Date().toISOString(); const contentType = response.headers.get("content-type")?.toLowerCase() ?? "";
  ...
  const probe = async (path: string) => { try { const target = validatePublicUrl(new URL(path, current).toString()); await assertPublicNetworkTarget(target); const result = await fetch(target, { redirect: "manual", signal: AbortSignal.timeout(3000), headers: { "user-agent": "INKSIGHTS-Visibility-Check/1.3" } }); const ok = result.ok; await result.body?.cancel(); return ok; } catch { return false; } };
...
Deno.serve(async req => {
  ...
  try {
    const contentLength = Number(req.headers.get("content-length") ?? 0); if (contentLength > MAX_BODY_BYTES) return json({ ok: false, error: "Submission is too large." }, 413);
    const rawBody = await req.text(); if (new TextEncoder().encode(rawBody).byteLength > MAX_BODY_BYTES) return json({ ok: false, error: "Submission is too large." }, 413);
    const body = JSON.parse(rawBody); if (clean(body.website_honeypot)) return json({ ok: false, error: "Invalid submission." }, 400);
    ...
    await assertPublicNetworkTarget(website); const websiteData = await snapshot(website); const token = crypto.randomUUID().replaceAll("-", "") + crypto.randomUUID().replaceAll("-", "");
    ...
    if (studioInsert.error) throw new Error(`studio_insert: ${studioInsert.error.message}`); studioId = studioInsert.data.id;
    ...
    const observationInsert = await sb.from("visibility_observations").insert({ studio_id: studioId, observation_type: "website", entity_name: website.toString(), source_provider: "studio_website", source_reference: websiteData.final_url, observed_at: websiteData.observed_at, raw_data: { ...websiteData, kind: "homepage_snapshot" } }); if (observationInsert.error) throw new Error(`website_observation: ${observationInsert.error.message}`);
    ...
    console.error(error);
    if (studioId) { try { const sb = createClient(Deno.env.get("SUPABASE_URL")!, Deno.env.get("SUPABASE_SERVICE_ROLE_KEY")!); await sb.from("visibility_studios").delete().eq("id", studioId); } catch (cleanupError) { console.error("visibility-cleanup", cleanupError); } }
