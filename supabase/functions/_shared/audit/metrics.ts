...
export function toAuditMetricRow(auditId: string, input: AuditMetricInput) {
  return {
    audit_id: auditId,
    ...
    evidence_classification: input.evidenceClassification,
    confidence: input.confidence,
    measurement_status: input.measurementStatus,
    not_measurable_reason: input.notMeasurableReason ?? null,
    required_source: input.requiredSource ?? null,
...
export function calculateTransactionMetrics(rows: ParsedTransaction[], period: TransactionMetricPeriod): AuditMetricInput[] {
  ...
  const provenance = { source_ids: period.sourceIds ?? [], grain: "payment_transaction", identity_resolution: "conservative_normalised_label" };
  const clientRows = new Map<string, ParsedTransaction[]>();
  for (const row of inPeriod) { const list = clientRows.get(row.normalisedClient) ?? []; list.push(row); clientRows.set(row.normalisedClient, list); }
  const repeatClients = new Set([...clientRows.entries()].filter(([, xs]) => xs.length >= 2).map(([client]) => client));
  const totalPence = inPeriod.reduce((sum, r) => sum + r.amountPence, 0);
  const repeatValuePence = inPeriod.filter((r) => repeatClients.has(r.normalisedClient)).reduce((sum, r) => sum + r.amountPence, 0);
  const beforeWindowClients = new Set(rows.filter((r) => r.date < period.reactivationWindowStart).map((r) => r.normalisedClient));
  const activeWindowClients = new Set(rows.filter((r) => r.date >= period.reactivationWindowStart && r.date <= period.periodEnd).map((r) => r.normalisedClient));
  ...
  for (const row of rows) { const year=Number(row.date.slice(0,4)); const list=byYear.get(year)??[]; list.push(row); byYear.set(year,list); }
  const partial = new Set(period.partialYears ?? []);
  for (const [year, yearRows] of [...byYear.entries()].sort(([a],[b])=>a-b)) {
    const yearTotal=yearRows.reduce((sum,row)=>sum+row.amountPence,0); const coverage=partial.has(year)?"PARTIAL_PERIOD":"FULL_YEAR";
...
export function calculateMissingOperationalMetrics(availableSources: string[]): AuditMetricInput[] {
  ...
  const missing = (metricKey:string, reason:string, requiredSource:string) => metric({ metricKey, evidenceClassification:"CALCULATED", confidence:"LOW", measurementStatus:"not_measurable", notMeasurableReason:reason, requiredSource, provenance:{available_sources:[...sourceSet],formula:"not calculated"} });
  ...
  if(!sourceSet.has("attendance")){ result.push(missing("no_show_rate","No attendance/no-show dataset is available.","attendance records")); result.push(missing("cancellation_rate","No cancellation dataset is available.","booking cancellation records")); }
